...
from models import Document, RBIClause
...
# ---------------------------------------------------------------------------
# GET /api/dev/document/{id}/parsed
# DEV ONLY
# ---------------------------------------------------------------------------
...
async def get_parsed_document(document_id: int, db: Session = Depends(get_db)):
    ...
    from document_parser import parse_pdf_with_pages, parse_docx_with_lines, clean_text
...
# ---------------------------------------------------------------------------
# POST /api/dev/rbi/analyze
# DEV ONLY — runs only the AI understanding step, no clause comparison
# ---------------------------------------------------------------------------
...
async def test_rbi_ai_analysis(db: Session = Depends(get_db)):
    ...
    from ai_analyzer import generate_rbi_understanding, get_active_provider, check_ai_connection
